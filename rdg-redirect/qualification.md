...
| Spiky / steep ramp-up traffic profile (still pending) | If scaling is fragile under sudden spikes, MCO may need a small uplift. Omar acknowledged this is **untested** and committed to running a steep-ramp-up load test. | Awaiting test results from Omar/Bountiful Cow. |
...
- **Load-test data (added 2026-05-06):** Omar reported the redirect proxy was load-tested at the rate limits in place; workers stayed below 30% utilisation. With 3 workers in prod, scaling kicks in at 800 TPS and prod CPU peaks <50%. **Steep ramp-up / spiky-traffic test still pending**, Omar committed to running it.
