...
Combined with the sublinear scaling (loi de puissance) from `item-types.md` :

```
MCO_for_N_resources_of_same_(type, coeff) = base_rate × scale(N, T) × coefficient

where scale(N, T) = T^(1−k) × N^k          (k ≈ 0.8 ; T = tenants_spanned, défaut 1)
```

Group ressources by **(item type, coefficient, tenants_spanned)** before applying the formula. The scaling captures the marginal cost of identical-profile ressources at scale, plus the multi-tenant fragmentation penalty (same exponent).
...
- 11 EC2 Debian medium across 3 envs:
  ...
  - Count: 11 (single-tenant) → scale = 11^0.8 = 6.81
  - MCO base = 0.1 × 6.81 × 0.8 = **0.545 j/h/mois**
